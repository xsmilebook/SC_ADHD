...
#' @export
#'
source("D:/code/R_projects/SC_ADHD/functions/help.R")

ComBat_sva <- function(dat, batch, mod = NULL, par.prior = TRUE, prior.plots = FALSE,
  ...
  zero.rows.lst <- lapply(levels(batch), function(batch_level){
    if(sum(batch==batch_level)>1){
      subset_dat <- as.matrix(dat[, batch==batch_level])
      return(which(apply(subset_dat, 1, function(x){var(x)==0})))
    }else{
  ...
  print(paste("gamma.bar:", gamma.bar))
  ...
  if (par.prior) {
    ...
    for (i in 1:n.batch) {
      gamma.star[i,] <- results[[i]]$gamma.star[2]
      delta.star[i,] <- results[[i]]$delta.star[2]
    }
  }
