...
set.seed(0925)
linearmediation <- function(dataname, M.var, Y.var, X.var, covariatesbase, covariates2year){
  ...
  #regress covariates
  Xcorformula <- as.formula(sprintf("%s ~ %s", X.var, covariatesbase))
  Mcorformula <- as.formula(sprintf("%s ~ %s", M.var, covariatesbase))
  Ycorformula <- as.formula(sprintf("%s ~ %s", Y.var, covariates2year))
  
  X.res<-residuals(gam(Xcorformula,method="REML", data = gam.data))
  M.res<-residuals(gam(Mcorformula,method="REML", data = gam.data))
  Y.res<-residuals(gam(Ycorformula,method="REML", data = gam.data))
