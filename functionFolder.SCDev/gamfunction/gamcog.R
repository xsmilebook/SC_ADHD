...
gam.fit.cognition <- function(region, dataname, cognition_var, smooth_var, covariates, knots,corrmethod, set_fx = FALSE, stats_only = FALSE){
  ...
  gam.cog.t <- gam.results$p.table[2,3]
  gam.cog.pvalue <- gam.results$p.table[2,4]
  
  #Full versus reduced model anova p-value
  #anova.cov.pvalue <- anova.gam(gam.model.null,gam.model,test='Chisq')$`Pr(>Chi)`[2]
  if (stats_only){
    anova.cov.pvalue <- anovaPB(gam.model.null,gam.model, n.sim = 1000,test='Chisq', ncpus=1)$`Pr(>Chi)`[2]
  ...
  anova.cov.int.pvalue <- anova.gam(gam.model,gam.model.int,test='Chisq')$`Pr(>Chi)`[2]
  ...
  ### effect direction
  if(gam.cog.t < 0){ #if the gam t-value for covariate of interest is less than 0, make the partialRsq negative
  ...
  stats.results <- cbind(parcel, cognition_var, gam.cog.t, gam.cog.pvalue,anova.cov.pvalue, anova.cov.int.pvalue,partialRsq.int, partialRsq, correstimate, corrp)
  data.results <- list()
