gam.varyingcoefficients <- function(region, dataname, smooth_var, int_var, covariates, knots, set_fx = FALSE, increments, draws, return_posterior_coefficients = FALSE){
  ...
  # AICresult <- AIC(gam.model.null, gam.model)
  # if (AICresult$AIC[1] < AICresult$AIC[2]){
  #   gam.model <- gam.model.null
  # }
  ...
  for (v in c(1:length(theseVars))) {
    ...
    if (thisVar == int_var) { 
      ...
      pred.low[,smooth_var] = seq(min(agevector,na.rm = T),max(agevector,na.rm = T), length.out = np)
    } else {
  ...
  for (v in c(1:length(theseVars))) {
    ...
    if (thisVar == int_var) { 
      ...
      pred.high[,smooth_var] = seq(min(agevector,na.rm = T),max(agevector,na.rm = T), length.out = np)
    } else {
      tab.tmp<-table(df[,thisVar])
      levelact<-which.max(tab.tmp)
      switch (thisClass,
