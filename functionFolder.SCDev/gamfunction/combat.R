...
combat <- function(dat, batch, mod=NULL, eb=TRUE, verbose=TRUE){
  ...
  .checkConstantRows <- function(dat){
    sds <- rowSds(dat) #Standard deviation estimates for each row (column) in a matrix.
    ns <- sum(sds==0)
    if (ns>0){
  ...
  n.array_N <- as.matrix(rep(1,n.array))
  s.data <- (dat-stand.mean)/(tcrossprod(sqrt(var.pooled), n.array_N))  
  ...
  delta.hat <- NULL
  for (i in batches){
    delta.hat <- rbind(delta.hat,matrixStats::rowVars(s.data, cols=i, na.rm=TRUE))
  }
  ...
  for (i in batches){
    if (eb){
      n.batches_N <-rep(1,n.batches[j])
      bayesdata[,i] <- (bayesdata[,i]-t(batch.design[i,]%*%gamma.star))/tcrossprod(sqrt(delta.star[j,]), n.batches_N)
      #bayesdata[,i] <- (bayesdata[,i]-t(batch.design[i,]%*%gamma.star))/tcrossprod(sqrt(delta.star[j,]), rep(1,n.batches[j]))
    } else {
      n.batches_N <-rep(1,n.batches[j])
      bayesdata[,i] <- (bayesdata[,i]-t(batch.design[i,]%*%gamma.hat))/tcrossprod(sqrt(delta.hat[j,]), n.batches_N)
      #bayesdata[,i] <- (bayesdata[,i]-t(batch.design[i,]%*%gamma.hat))/tcrossprod(sqrt(delta.hat[j,]), rep(1,n.batches[j]))
    }
    j <- j+1
  }
  n.array_N <- as.matrix(rep(1,n.array))
  bayesdata <- (bayesdata*(tcrossprod(sqrt(var.pooled), n.array_N)))+stand.mean
